...
for (om in 1:24) {
  ...
  # mod.tmp <- mods_list[[i]][[j]]
  Catch <- sapply(1:50,function(i) {
    mod.tmp <- mods_list[[i]][[1]]
    Catch <- mod.tmp$input$data$agg_catch
  })
  library(dplyr)
  library(tidyr)
...
write.csv(res,"data.csv",row.names = F)

ggplot(res,aes(x = Year, y = Catch, group = nsim)) +
  geom_line(alpha = 0.5) + 
